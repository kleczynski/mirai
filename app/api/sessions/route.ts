...
function withChatFlag<T extends Session>(s: T) {
  return { ...s, discoveryChatEnabled: env.MIRAI_DISCOVERY_ENABLED === "true" };
}
export async function GET(request: Request) {
  try {
    ...
    const rows = await database()
      .prepare(
        "SELECT * FROM sessions WHERE owner_id = ? ORDER BY updated_at DESC LIMIT 500",
      )
      .bind(u.userId)
      .all<Row>();
    return json(
      rows.results.map((row) => {
        const s = unpack(row);
        return withChatFlag(
          s.discovery ? { ...s, discovery: { ...s.discovery, transcript: [] } } : s,
        );
      }),
    );
  } catch (e) {
...
const creation = z.object({
  ...
  template: z.enum(["custom", "carpenter", "retail", "dental"]),
  language: z.enum(["en", "pl"]),
});
export async function POST(request: Request) {
  try {
    const u = await owner();
    const data = creation.parse(await body(request));
...
const mutation = z.discriminatedUnion("action", [
  z.object({
    action: z.literal("confirm-discovery"),
    id: z.string().uuid(),
    revision: z.number().int().nonnegative(),
  }),
  z.object({ action: z.literal("invite"), id: z.string().uuid() }),
  ...
  z.object({
    ...
    id: z.string().uuid(),
    url: z.string().max(2000).refine(validDemoUrl),
    summary: z.string().trim().min(10).max(4000),
    checks: z.array(z.string()).refine((c) => demoChecks.every((k) => c.includes(k))),
  }),
]);
  ...
  try {
    ...
    if (m.action === "confirm-discovery") {
      try {
        ...
        if (e instanceof DiscoveryError) throw new ApiError(e.status, e.message);
        throw e;
      }
    ...
    if (m.action === "invite" || m.action === "revoke") {
      ...
      const expiresAt = new Date(
        m.action === "revoke" ? 0 : Date.now() + 30 * 86400000,
      ).toISOString();
      await database()
        .prepare(
          "UPDATE sessions SET token_hash = ?, expires_at = ?, revision = revision + 1 WHERE id = ?",
        )
        .bind(await hash(token), expiresAt, s.id)
        .run();
    ...
    if (!isDiscoveryComplete(s))
      throw new ApiError(409, "Finish discovery before sharing a demo.");
    const demo = {
      id: crypto.randomUUID(),
      version: s.demos.length + 1,
      url: m.url,
      summary: m.summary,
      checks: m.checks,
