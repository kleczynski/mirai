import { z } from 'zod';
import { allowedQuestions, hostQuestions, openGaps, type QuestionId, type TurnMeta, interviewComplete, finishInterview, isFinishIntent, DISCOVERY_PROMPT_VERSION, applyModelOutput, correctTopic, evidenceReady, hostTurn, initialDiscovery, parseModelOutput, topicKeys } from './discovery';
import { callOpenAI, PROVIDER_TIMEOUT_MS, DISCOVERY_MODEL, providerPayload, reserveCost, type DiscoveryConfig, type Provider } from './discovery-provider';
...
function updateStatement(db: D1Database, s: Session, next: Session, tokenHash: string, afterAdmission = false) {
  return db.prepare("UPDATE sessions SET data = ?, revision = revision + 1, updated_at = ? WHERE id = ? AND revision = ? AND token_hash = ? AND expires_at > ? AND json_array_length(json_extract(data, '$.demos')) = 0 AND json_extract(data, '$.source') IS NULL" + (afterAdmission ? " AND changes() = 1" : ""))
...
export async function recoverDiscoveryProcessing(db: D1Database, s: Session, tokenHash: string): Promise<Session> {
  const pending = s.discovery?.processing;
  if (pending?.status !== 'pending' || Date.now() - Date.parse(pending.startedAt) < PROCESSING_LEASE_MS) return s;
  const next = failedProcessing(s);
  const result = await db.batch([
    updateStatement(db, s, next, tokenHash),
    db.prepare("UPDATE discovery_requests SET status = 'failed', metadata = ? WHERE id = ? AND status = 'pending' AND changes() = 1").bind(JSON.stringify({ model: DISCOVERY_MODEL, promptVersion: DISCOVERY_PROMPT_VERSION, completionReason: 'processing_expired' }), `${s.id}:${pending.requestId}`),
  ]);
...
export async function handleDiscovery(request: Request, db: D1Database, config: DiscoveryConfig, provider: Provider = callOpenAI, signal?: AbortSignal, defer?: (work: Promise<unknown>) => void): Promise<Session> {
  const assertActive = () => { if (signal?.aborted) throw new DiscoveryError(504, 'The request timed out. Your draft is kept; refresh before retrying. / Przekroczono czas. Szkic zachowany; odśwież przed ponowieniem.'); };
  assertActive();
  ...
  if (m.action === 'message') {
    ...
    if (prior) {
      ...
      if (prior.status === 'saved' || s.discovery?.transcript.some(t => t.id === m.requestId && t.kind === 'answer')) return s;
    ...
    const stopped = s.discovery?.transcript.find(t => t.id === m.requestId && t.kind === 'finish');
    if (stopped) { if (stopped.text === m.text) return s; throw conflict(pl); }
  } else if (s.discovery?.transcript.some(t => t.id === m.requestId)) {
    const t = s.discovery.transcript.find(t => t.id === m.requestId)!;
    if ((m.action === 'finish' && t.kind === 'finish') || (m.action === 'edit' && t.text === m.text && t.topic === m.topic) || (m.action === 'edit-message' && t.text === m.text && t.supersedes?.includes(m.messageId)) || (m.action === 'path' && t.text === m.path && t.topic === 'path')) return s;
    throw conflict(pl);
  }
  ...
  if (m.action === 'start') {
  ...
  } else if (m.action === 'finish' || m.action === 'message' && isFinishIntent(m.text)) {
    next.discovery = structuredClone(s.discovery!);
    next.discovery.transcript.push({ id: m.requestId, role: m.action === 'message' ? 'client' : 'system', text: m.action === 'message' ? m.text : 'finish', kind: 'finish', createdAt: now });
    next.discovery = finishInterview(next.discovery, now, s.language, 'client_finished');
    ...
    next.discovery.transcript.push({ id: m.requestId, role: 'client', text: m.text, kind: 'correction', createdAt: now, supersedes: [original.id] });
    ...
    if (s.discovery!.processing?.status === 'pending') throw new DiscoveryError(429, pl ? 'Odpowiedź jest zapisana. Poczekaj na podsumowanie lub użyj formularza tematów.' : 'Your answer is saved. Wait for the summary or use the topic form.');
    if (interviewComplete(s.discovery!)) throw new DiscoveryError(409, pl ? 'Rozmowa zakończona. Popraw odpowiedzi w formularzu tematów.' : 'The interview is finished. Edit answers in the topic form.');
    ...
    d.transcript.push({ id: m.requestId, role: 'client', text: m.text, kind: 'answer', createdAt: now });
    ...
    const admitted = await db.batch([
      db.prepare(`INSERT INTO discovery_requests (id, session_id, fingerprint, status, reserved_microusd, created_at)
      ...
      ON CONFLICT(id) DO NOTHING`).bind(id, s.id, fingerprint, reserved, millis, s.id, s.revision, tokenHash, now, s.id, reserved, Math.min(1_000_000, config.capMicrousd), reserved, Math.min(2_000_000, config.workspaceCapMicrousd ?? 2_000_000), s.id, s.id, millis - 6000, millis - 60000),
      updateStatement(db, s, next, tokenHash, true),
    ]);
    if (!admitted[0].meta.changes) throw new DiscoveryError(429, pl ? 'Poczekaj chwilę przed kolejną wiadomością. Jeśli limit rozmowy został osiągnięty, użyj formularza tematów.' : 'Wait a moment before another message. If this conversation has reached its usage limit, use the topic form.');
    if (!admitted[1].meta.changes) {
      await db.prepare("UPDATE discovery_requests SET status = 'conflict', charged_microusd = 0 WHERE id = ?").bind(id).run();
      throw conflict(pl);
    }
    const accepted: Session = { ...next, revision: s.revision + 1 };
    const complete = async (): Promise<Session> => {
      let charged: number | null = null;
      let meta: TurnMeta = { model: DISCOVERY_MODEL, promptVersion: DISCOVERY_PROMPT_VERSION, completionReason: 'provider_failure', latencyMs: 0 };
      let final: Session;
      ...
      try {
        const result = await Promise.race([
          ...
          new Promise<never>((_, reject) => { timer = setTimeout(() => { controller.abort(); reject(new Error('provider_timeout')); }, PROVIDER_TIMEOUT_MS); }),
        ]);
        charged = result.chargedMicrousd; meta = result.meta;
        // A lost Worker or stale lease must never deliver a later result.
        if (Date.now() - millis >= PROCESSING_LEASE_MS) throw new Error('provider_timeout');
        meta = { ...meta, completionReason: 'validation_failed' };
        const out = parseModelOutput(result.output, d, s, m.requestId);
        const discovery = applyModelOutput(d, out, new Date().toISOString(), result.meta, s);
        delete discovery.processing;
        final = { ...accepted, discovery, updatedAt: new Date().toISOString() };
        meta = result.meta;
      } catch (error) {
        const timedOut = controller.signal.aborted || error instanceof Error && (error.name === 'TimeoutError' || error.message === 'provider_timeout');
        const reasons = ['duplicate topic', 'stale evidence', 'unsourced quote', 'client correction protected', 'use explicit path control', 'unsourced path'];
        if (meta.completionReason === 'validation_failed') meta = { ...meta, validationReason: error instanceof z.ZodError ? 'schema' : error instanceof Error && reasons.includes(error.message) ? error.message : 'invalid_output' };
        meta = { ...meta, completionReason: timedOut ? 'provider_timeout' : meta.completionReason, latencyMs: Date.now() - millis };
        final = failedProcessing(accepted);
        failure = new DiscoveryError(meta.completionReason === 'validation_failed' ? 502 : 503, pl ? 'Odpowiedź jest zapisana, ale podsumowanie nie powstało. Przejrzyj historię i uzupełnij formularz tematów.' : 'Your answer is saved, but the summary could not be prepared. Review the history and fill in the topic form.');
      } finally { clearTimeout(timer); }
      const batch = await db.batch([
        updateStatement(db, accepted, final, tokenHash),
        db.prepare("UPDATE discovery_requests SET status = CASE WHEN changes() = 1 THEN ? ELSE 'conflict' END, charged_microusd = ?, metadata = ? WHERE id = ?").bind(failure ? 'failed' : 'saved', charged, JSON.stringify(meta), id),
      ]);
  ...
  if (next.discovery && !next.discovery.interview) {
    if (evidenceReady(next.discovery)) next.discovery = finishInterview(next.discovery, now, s.language, 'coverage');
    else if (m.action === 'edit' || m.action === 'path' || m.action === 'edit-message') {
      const activeId = next.discovery.transcript.findLast(t => t.role === 'assistant' && t.meta?.questionId)?.meta?.questionId as QuestionId | undefined;
      if (activeId && hostQuestions[activeId] && !openGaps(next.discovery).includes(hostQuestions[activeId].topic)) {
        const question = allowedQuestions(next.discovery, s.template)[0];
        if (question) next.discovery.transcript.push(hostTurn(hostQuestions[question][s.language], now, question));
        else next.discovery = finishInterview(next.discovery, now, s.language, 'questions_exhausted');
      }
    }
  }
  assertActive();
...
export function confirmDiscovery(s: Session, revision: number) {
  ...
  if (s.demos.length || s.source || !s.discovery || s.discovery.processing?.status === 'pending' || !evidenceReady(s.discovery)) throw new DiscoveryError(409, 'Required discovery evidence is still missing or discovery is read-only.');
