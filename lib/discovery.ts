...
export type TurnMeta = { model: string; promptVersion: string; latencyMs: number; completionReason: string; inputTokens?: number; outputTokens?: number; costMicrousd?: number; validationReason?: string; questionId?: string };
export type DiscoveryTurn = { id: string; role: 'assistant' | 'client' | 'system'; text: string; createdAt: string; topic?: DiscoveryTopic; supersedes?: string[]; kind?: 'answer' | 'correction' | 'topic_edit' | 'finish'; meta?: TurnMeta };
export type Discovery = { version: 1; path: DiscoveryPath | null; topics: Partial<Record<DiscoveryTopic, Evidence>>; transcript: DiscoveryTurn[]; suggestedCompleteness: number; confirmedAt: string | null; processing?: { requestId: string; status: 'pending' | 'failed'; startedAt: string }; interview?: { status: 'review'; reason: 'client_finished' | 'answer_limit' | 'coverage' | 'questions_exhausted' | 'processing_failed'; completedAt: string } };
...
export function answeredQuestions(d: Discovery) {
  return d.transcript.filter(t => t.role === 'client' && (t.kind === 'answer' || !t.kind && !t.topic && !t.supersedes?.length && !t.id.startsWith('legacy-'))).length;
}
export function interviewComplete(d: Discovery) { return !!d.interview || answeredQuestions(d) >= MAX_DISCOVERY_ANSWERS || evidenceReady(d); }
export function isFinishIntent(text: string) {
  const value = text.trim().toLowerCase().replace(/[.!?]+$/g, '').replace(/[’]/g, "'");
  return /^(?:please\s+)?(?:finish(?: now)?|stop(?: now)?|that's (?:enough|all(?: i have| for now)?|everything(?: you need)?)|that is (?:enough|all(?: i have| for now)?|everything(?: you need)?)|i(?:'m| am) done(?: with (?:the )?(?:interview|questions))?|no more questions|end (?:the )?(?:interview|conversation)|to (?:wszystko|wystarczy)|wystarczy|koniec|zakończ(?: teraz| rozmowę)?|skończ(?:my)?|nie mam nic więcej(?: do dodania)?)(?:$|[,;:]\s*|\s+(?:please|proszę)\b)/.test(value);
}
function quotesFor(d: Discovery, topic: DiscoveryTopic) { return d.topics[topic]?.clientQuotes.join(' ') ?? ''; }
export function evidenceBlockers(d: Discovery): { topic: DiscoveryTopic; code: string; en: string; pl: string }[] {
  ...
  if (/\d+(?:[.,]\d+)?\s*(?:%|percent|procent)/i.test(success)) {
    const sample = /sample|dataset|cases|examples|prób|przypadk|zestaw|przykład/i.test(success);
    const truth = /label|ground truth|reference|manual|review|expert|compare|etykiet|referenc|ręczn|ekspert|porówn|sprawdz/i.test(success);
    const method = /correct|incorrect|false|precision|recall|pass|fail|numerator|denominator|poprawn|błęd|trafn|licznik|mianownik/i.test(success);
    if (!sample || !truth || !method || /not (?:defined|specified|known|established)|undefined|unspecified|unknown|nie (?:ustal|określ)|brak/i.test(success)) blockers.push({ topic: 'success_criteria', code: 'metric_evaluation', en: 'UNCONFIRMED — define the evaluation sample, reference reviewer/labels and how correct and failed results produce the claimed success rate.', pl: 'UNCONFIRMED — ustal próbkę, wzorzec lub osobę oceniającą i sposób obliczania wyniku z poprawnych i błędnych wyników.' });
  }
  const currentMessages = d.transcript.filter(t => t.role === 'client' && !d.transcript.some(replacement => replacement.supersedes?.includes(t.id)) && (!t.topic || d.topics[t.topic]?.sourceIds.includes(t.id)));
  const evidence = [...Object.values(d.topics).flatMap(e => e.clientQuotes), ...currentMessages.map(t => t.text)].join(' ');
  const tools = d.topics.tools_data;
  const cameraRemoved = tools?.origin === 'client' && /(?:camera|video|kamer[aęy]?|cctv) (?:removed|out of scope|not used|usunięt)|(?:no|without) (?:live )?(?:camera|video)|bez kamer/i.test(tools.clientQuotes.join(' '));
  if (!cameraRemoved && /camera|video|kamer|wizyjn|cctv/i.test(evidence)) {
    const constraints = quotesFor(d, 'constraints');
    const checks = [
      ['privacy', /privacy|private|personal|prywat|osobow/i, 'privacy boundaries', 'zasady prywatności'],
      ['consent', /consent|permission|zgod|pozwolen/i, 'participant consent or an approved basis for capture', 'zgody uczestników lub zatwierdzoną podstawę rejestracji'],
      ['retention', /retention|delete|retain|storage duration|retenc|usuw|przechow/i, 'retention and deletion policy', 'zasady retencji i usuwania'],
      ['safety', /safety|safe|machine control|physical|bezpiecz|sterow|maszyn/i, 'operational safety boundaries', 'granice bezpieczeństwa operacyjnego'],
      ['oversight', /human|operator|review|manual|człowiek|człowieka|nadzór|sprawdz|ręczn/i, 'human oversight', 'nadzór człowieka'],
      ['failure', /fail|outage|uncertain|unavailable|false|awari|błęd|niepewn|niedostępn/i, 'failure and uncertain-result handling', 'obsługę awarii i niepewnych wyników'],
    ] as const;
    for (const [code, pattern, en, pl] of checks) if (!pattern.test(constraints) || /not (?:defined|specified|known|established)|undefined|unspecified|unknown|nie (?:ustal|określ)/i.test(constraints) || /(?:no|without|ignore|unknown|unspecified|brak|bez|nie znam)\s+(?:(?:any|human|operational)\s+)?(?:privacy|consent|retention|safety|oversight|failure|nadzoru|prywatności|bezpieczeństwa)/i.test(constraints)) blockers.push({ topic: 'constraints', code, en: `UNCONFIRMED — camera workflow needs ${en}.`, pl: `UNCONFIRMED — przepływ z kamerą wymaga ustaleń obejmujących ${pl}.` });
  }
  return blockers;
}
export function openGaps(d: Discovery): DiscoveryTopic[] {
  return [...new Set([...requiredTopics(d).filter(k => !d.topics[k]?.summary.trim() || d.topics[k]?.confidence === 'low' || k === 'path' && !d.path), ...evidenceBlockers(d).map(b => b.topic)])];
}
export function evidenceReady(d: Discovery) { return openGaps(d).length === 0; }
export function finishInterview(d: Discovery, now: string, language: 'en' | 'pl', reason: NonNullable<Discovery['interview']>['reason']): Discovery {
  ...
  next.transcript.push(hostTurn(language === 'pl'
    ...
    : `The interview is finished. Review your saved answers.${gaps.length ? ' Add missing information directly in the topic form.' : ''} Your host still needs to confirm the evidence.`, now));
  return next;
}
export function initialDiscovery(s: SessionData, now: string): Discovery {
  ...
  if (!first) return finishInterview(d, now, s.language, evidenceReady(d) ? 'coverage' : 'questions_exhausted');
  d.transcript.push(hostTurn(first === 'context' ? greeting(s.language) : hostQuestions[first][s.language], now, first));
  return d;
}
export function greeting(language: 'en' | 'pl') { return language === 'pl' ? 'Cześć, tu Mirai. Możemy usprawnić codzienną pracę albo odkryć zupełnie nowy pomysł. Jak wygląda Twoja praca i co chcesz w niej zmienić?' : 'Hi, I’m Mirai. We can improve everyday work or explore something new together. What does your work look like and what would you like to change?'; }
...
export const hostQuestions = {
  evaluation: { topic: 'success_criteria', en: 'How will you evaluate success on a defined sample against a reviewed reference, counting both correct and failed results?', pl: 'Jak ocenisz sukces na ustalonej próbce względem sprawdzonego wzorca, licząc poprawne i błędne wyniki?' },
  safeguards: { topic: 'constraints', en: 'What operating policy should cover camera privacy, retention, human oversight, safe limits and failure recovery?', pl: 'Jakie zasady mają obejmować prywatność kamery, retencję, nadzór człowieka, bezpieczne granice i obsługę awarii?' },
  context: { topic: 'context', en: 'What does your work look like and what would you like to change?', pl: 'Jak wygląda Twoja praca i co chcesz w niej zmienić?' },
  ...
  workflow: { topic: 'workflow', en: 'Walk me through one case from its trigger to the final result: what happens along the way?', pl: 'Przejdźmy przez jeden przypadek od początku do wyniku: co dzieje się po drodze?' },
  workflow_next: { topic: 'workflow', en: 'What happens next?', pl: 'Co dzieje się potem?' },
  frequency: { topic: 'frequency_impact', en: 'What does a typical week of this work look like in volume, time and impact?', pl: 'Jak wygląda typowy tydzień tej pracy pod względem liczby przypadków, czasu i skutków?' },
  ...
  success: { topic: 'success_criteria', en: 'What observable result would make the first version successful, and how would you check it?', pl: 'Jaki obserwowalny wynik oznacza sukces pierwszej wersji i jak go sprawdzisz?' },
  ...
  delivery: { topic: 'delivery', en: 'What first-use scenario should we test, including the person, device and expected result?', pl: 'Jaki scenariusz pierwszego użycia mamy przetestować, uwzględniając osobę, urządzenie i oczekiwany wynik?' },
...
export function allowedQuestions(d: Discovery, template: SessionData['template']): QuestionId[] {
  ...
  const asked = new Set(d.transcript.filter(t => t.role === 'assistant' && t.meta?.questionId).map(t => t.meta!.questionId!));
  const askedTopics = new Set([...asked].filter(id => id in hostQuestions).map(id => hostQuestions[id as QuestionId].topic));
  const gaps = new Set(openGaps(d));
  const problem: QuestionId = template !== 'custom' ? template : d.path === 'creative' ? 'idea' : 'task';
  const order: QuestionId[] = ['context', problem, 'path', 'workflow', 'frequency', 'example', 'success', 'constraints', 'delivery'];
  const candidates = order.filter(id => gaps.has(hostQuestions[id].topic) && !asked.has(id) && !askedTopics.has(hostQuestions[id].topic));
  // One specific follow-up may resolve a measurable/safety gap; never repeat a broad prompt.
  const blockers = evidenceBlockers(d);
  if (blockers.some(b => b.topic === 'success_criteria') && !asked.has('evaluation')) candidates.unshift('evaluation');
  if (blockers.some(b => b.topic === 'constraints') && !asked.has('safeguards')) candidates.unshift('safeguards');
  return candidates;
}
...
const evidenceOutputSchema = modelOutputSchema.pick({ topicUpdates: true, path: true }).strict();
export type ModelOutput = z.infer<typeof modelOutputSchema> | z.infer<typeof evidenceOutputSchema>;
export function parseModelOutput(raw: unknown, d: Discovery, s: Pick<SessionData, 'language' | 'template'>, latestId: string): ModelOutput {
  ...
  for (const update of out.topicUpdates) {
    ...
    for (const quote of update.quotes) {
      ...
      if (!message || d.transcript.some(t => !t.topic && t.supersedes?.includes(message.id)) || !message.text.includes(quote.text)) throw new Error('unsourced quote');
    }
...
export function applyModelOutput(d: Discovery, out: ModelOutput, now: string, meta: TurnMeta, s: Pick<SessionData, 'language' | 'template'> = { language: 'en', template: 'custom' }): Discovery {
  const next = structuredClone(d); next.confirmedAt = null;
  ...
  next.suggestedCompleteness = 'suggestedCompleteness' in out ? out.suggestedCompleteness : 1 - openGaps(next).length / requiredTopics(next).length;
  if (answeredQuestions(next) >= MAX_DISCOVERY_ANSWERS) return finishInterview(next, now, s.language, 'answer_limit');
  ...
  next.transcript.push({ id: crypto.randomUUID(), role: 'assistant', text: hostQuestions[questionId][s.language], createdAt: now, meta: { ...meta, questionId } });
  return next;
}
export function correctTopic(d: Discovery, topic: DiscoveryTopic, text: string, id: string, now: string): Discovery {
  const next = structuredClone(d); next.confirmedAt = null; delete next.processing;
  next.transcript.push({ id, role: 'client', text, topic, kind: 'topic_edit', createdAt: now, supersedes: next.topics[topic]?.sourceIds ?? [] });
