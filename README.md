...
Owner UI signs in at `/sign-in` and signs out through Clerk. The portable profile still simulates ChatGPT sign-in for loopback HTTP tests only: visit `/signin-with-chatgpt?return_to=/` to sign in as `local_seedy` (`seedy@sites.test`, display name `Seedy`) and `/signout-with-chatgpt?return_to=/` to sign out. That mock is disabled in the managed-linux profile and is not owner identity in production builds. A hosted Worker must not treat `oai-authenticated-user-*` as identity.
