...
const checks = ['Core client journey tested', 'Fictional or approved demo data only', 'Mobile layout and empty states checked', 'Client access tested in a signed-out browser'];
...
r = await call('/api/sessions', 'POST', { title: 'Lifecycle smoke session', client: 'Fictional smoke client', template: 'custom', language: 'en' });
...
for (const key of keys) {
  ...
  r = await call('/api/client', 'POST', { action: 'answer', revision: current.value.revision, key, answer: 'Fictional lifecycle evidence for ' + key }, active, false);
...
r = await call('/api/sessions', 'PATCH', { action: 'demo', id, url: 'javascript:alert(1)', summary: 'Try the fictional first-use path.', checks });
assert.equal(r.status, 400, '7. incomplete/unsafe attach is 400');
r = await call('/api/sessions', 'PATCH', { action: 'demo', id, url: 'https://example.com/test-try', summary: 'short', checks });
assert.equal(r.status, 400);
r = await call('/api/sessions', 'PATCH', { action: 'demo', id, url: 'https://example.com/test-try', summary: 'Try the fictional first-use path.', checks: [] });
assert.equal(r.status, 400);
r = await call('/api/client', 'GET', undefined, active, false);
assert.equal(r.value.stage, beforeAttach, '7. stage unchanged after incomplete attach');

r = await call('/api/sessions', 'PATCH', { action: 'demo', id, url: 'https://example.com/test-try', summary: 'Try the fictional first-use path on this version.', checks });
assert.equal(r.status, 200, JSON.stringify(r));
r = await call('/api/client', 'GET', undefined, active, false);
...
r = await call('/api/client', 'POST', { action: 'feedback', demoId, kind: 'note', text: 'Fictional note on the current version.', name: 'Smoke client' }, active, false);
assert.equal(r.status, 200, '8. client note on current demo');
r = await call('/api/client', 'POST', { action: 'feedback', demoId, kind: 'approval', text: 'Approve this fictional version.', name: 'Smoke client' }, active, false);
assert.equal(r.status, 200, '8. client approval on current demo');
r = await call('/api/client', 'GET', undefined, active, false);
assert.equal(r.value.approvedDemoId, demoId);

r = await call('/api/sessions', 'PATCH', { action: 'demo', id, url: 'https://example.com/test-try-v2', summary: 'Second fictional hosted version for approval reset.', checks });
assert.equal(r.status, 200);
r = await call('/api/client', 'GET', undefined, active, false);
assert.equal(r.value.approvedDemoId, null, '8. second attach clears approval');
assert.equal(r.value.demos.length, 2);
r = await call('/api/client', 'POST', { action: 'feedback', demoId, kind: 'note', text: 'Stale version note', name: 'Smoke client' }, active, false);
assert.equal(r.status, 409, '8. old demoId feedback is 409');

console.log('Passed: localhost lifecycle smoke covers anonymous 401, invite-once, client vs owner views, rotation, revoke, eight answers, attach blockers, Demo review without bundled, note+approval, and approval reset.');
