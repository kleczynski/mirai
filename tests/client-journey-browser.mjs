...
try {
  ...
  console.log(
    "Skipped client-journey-browser: install playwright as a local optional dependency to run this smoke.",
  );
  process.exit(0);
}
...
const created = await call("/api/sessions", "POST", {
  ...
  client: "Fictional browser client",
  template: "custom",
  language: "en",
});
...
for (const key of keys) {
  const current = await fetch(origin + "/api/client", {
    headers: { Authorization: `Bearer ${token}` },
  });
  const session = await current.json();
  const saved = await fetch(origin + "/api/client", {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...
      key,
      answer: "Fictional browser evidence for " + key,
    }),
  });
...
try {
  ...
  assert.equal(
    await clientPage.locator(".workspace-main").count(),
    0,
    "signed-out /s#token is the client page, not the workspace",
  );
  await client.close();
  ...
  await ownerPage
    .getByLabel("What should your client try?")
    .fill("Try the fictional first-use path.");
  for (const label of [
  ...
  await ownerPage.waitForFunction(() => {
    const button = [...document.querySelectorAll("button")].find((node) =>
      node.textContent?.includes("Share this version"),
    );
...
console.log(
  "Passed localhost browser smoke: client page isolation, attach-dialog blockers, and /test-try 404. Invitation token was not printed.",
);
