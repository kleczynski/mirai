...
const created = await fetch(origin + '/api/sessions', {
  method: 'POST', headers: { cookie, 'Content-Type': 'application/json', Origin: origin },
  body: JSON.stringify({ title: 'Fictional boundary test', client: 'Fictional reviewer', template: 'custom', language: 'en' }),
});
...
try {
  const response = await fetch(origin + '/api/client/discovery-chat', {
    ...
    body: new ReadableStream({ start(c) { stream = c; c.enqueue(new TextEncoder().encode('{')); } }),
  ...
  console.log(`Passed local route deadline: usable recovery in ${Math.round(elapsed)}ms; no model call or answer persisted.`);
} finally {
  clearTimeout(safetyTimer);
  try { stream?.close(); } catch { /* Fetch may already have cancelled upload. */ }
  controller.abort();
}
