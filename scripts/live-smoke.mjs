...
if (confirmation !== "I_UNDERSTAND") {
  console.error(
    "Refusing to run. Set MIRAI_LIVE_SMOKE=I_UNDERSTAND and MIRAI_LIVE_SMOKE_ORIGIN explicitly. Default origin is localhost. This script is not in CI.",
  );
  process.exit(1);
}
...
if (!process.env.MIRAI_LIVE_SMOKE_ORIGIN) {
  console.error(
    "Refusing to run without an explicit MIRAI_LIVE_SMOKE_ORIGIN. Default would be http://localhost:5173; set that origin if that is what you intend.",
  );
  process.exit(1);
}

if (
  /^https:\/\/mirai\.party\/?$/i.test(origin) &&
  process.env.MIRAI_LIVE_SMOKE_PRODUCTION !== "I_REALLY_MEAN_MIRAI_PARTY"
) {
  console.error(
    "Refusing https://mirai.party unless MIRAI_LIVE_SMOKE_PRODUCTION=I_REALLY_MEAN_MIRAI_PARTY is also set.",
  );
  process.exit(1);
}
...
const sign = await fetch(
  origin.replace(/\/$/, "") + "/signin-with-chatgpt?return_to=/",
  { redirect: "manual" },
);
const cookie = sign.headers.get("set-cookie")?.split(";")[0];
if (!cookie) {
  console.error(
    "No loopback owner cookie. Hosted Workers are Clerk-only; this optional smoke is for localhost or an operator-approved host that still exposes the loopback mock.",
  );
  process.exit(1);
}
...
async function call(path, method = "GET", data, token, operator = true) {
  const r = await fetch(origin.replace(/\/$/, "") + path, {
    ...
    headers: {
      ...
      ...(data
        ? { "Content-Type": "application/json", Origin: origin.replace(/\/$/, "") }
        : {}),
    },
...
r = await call("/api/sessions", "POST", {
  title: "Optional live smoke",
  client: "Fictional live smoke",
  template: "custom",
  language: "en",
});
