...
const cases = [
  {
    name: "english-camera",
    language: "en",
    text: "Fictional test: I supervise a packing line and want to automate recurring defect checks using a live camera feed. Workers currently inspect boxes by eye for eight hours daily. I want WhatsApp alerts for defects, with 95% success. No boundaries have been decided. A supervisor would try it in a phone browser.",
  },
  {
    name: "polish-workshop",
    language: "pl",
    text: "Przykład fikcyjny: prowadzę warsztaty plastyczne i chcę usprawnić powtarzalne zapisy. Uczestnik wysyła zgłoszenie, zapisuję je w arkuszu i ręcznie potwierdzam miejsce. To 20 zapisów i dwie godziny pracy tygodniowo. Chcę formularz i listę miejsc bez integracji. Koordynator sprawdzi na telefonie 10 fikcyjnych zgłoszeń: każde ma pojawić się na liście tylko raz, a jedenasty zapis przy limicie 10 ma zostać odrzucony. Porówna wyniki z ręcznie przygotowaną listą. Dane są prywatne, tylko dla koordynatora, usuwane po 7 dniach. Przy awarii zachowujemy szkic i prosimy o ponowny zapis; nie wysyłamy wiadomości automatycznie.",
  },
];
...
if (process.env.MIRAI_PAID_TEST_CALLS !== "2") {
  console.error(
    "Refusing paid calls. Set MIRAI_PAID_TEST_CALLS=2 only with explicit authorization.",
  );
  process.exit(1);
}
let key = process.env.MIRAI_OPENAI_API_KEY ?? "";
for (const file of [".dev.vars", ".env.local"]) {
  if (!key && existsSync(root + file))
    key = parseEnv(readFileSync(root + file, "utf8")).MIRAI_OPENAI_API_KEY ?? "";
}
if (!key.trim()) {
  console.error(
    "No discovery API key configured. Add MIRAI_OPENAI_API_KEY to ignored .dev.vars. No paid calls made.",
  );
  process.exit(1);
}
...
if (previous?.attempts?.length) {
  console.error(
    "This two-call run already started. Inspect the saved report; this script never repeats paid attempts automatically.",
  );
  process.exit(1);
}
try {
  writeFileSync(lockPath, "Two-call live test in progress.\n", {
    flag: "wx",
    mode: 0o600,
  });
} catch {
  console.error("A paid test lock already exists. No new calls made.");
  process.exit(1);
}
const report = {
...
try {
  ...
  for (const fixture of cases) {
    ...
    const request = (data) =>
      new Request("http://localhost/api/client/discovery-chat", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}`, Origin: "http://localhost" },
        body: JSON.stringify(data),
      });
    const started = await handleDiscovery(
      request({ action: "start", revision: 0, requestId: crypto.randomUUID() }),
      db,
      config,
    );
    let attempt;
    ...
    const liveProvider = async (payload, apiKey, signal) => {
      ...
      const spent = readdirSync(root + "outputs")
        ...
        .flatMap(
          (name) =>
            JSON.parse(readFileSync(root + "outputs/" + name, "utf8")).attempts ?? [],
        )
        .reduce(
          (total, a) =>
            total +
      ...
      if (spent + reservedMicrousd > 1_000_000)
        throw new Error("cumulative_paid_smoke_budget");
      attempt = {
        reservedMicrousd,
        number: report.attempts.length + 1,
        case: fixture.name,
        status: "started",
        startedAt: new Date().toISOString(),
      };
    ...
    try {
      let job;
      const accepted = await handleDiscovery(
        request({
          action: "message",
      ...
      attempt.checks = {
        ...
        operatorGatePreserved:
          saved.stage === "Discovery" && !saved.discovery.confirmedAt,
        directionNotRepeated: saved.discovery.path
          ? saved.discovery.transcript.at(-1)?.meta?.questionId !== "path"
          : false,
      };
      if (fixture.name === "english-camera") {
        ...
        attempt.checks.cameraSafeguardsBlocked = evidenceBlockers(saved.discovery).some(
          (b) => b.topic === "constraints",
        );
      }
      attempt.topicConfidence = Object.fromEntries(
        Object.entries(saved.discovery.topics).map(([topic, evidence]) => [
          topic,
          evidence.confidence,
        ]),
      );
      attempt.nextQuestion =
        saved.discovery.transcript.at(-1)?.meta?.questionId ?? "review";
      ...
      attempt.applicationStatus =
        error instanceof DiscoveryError ? error.status : "unexpected";
    ...
    const ledger = sqlite
      .prepare(
        "SELECT status, reserved_microusd, charged_microusd, metadata FROM discovery_requests WHERE id = ?",
      )
      .get(`${id}:${requestId}`);
    const meta = ledger ? JSON.parse(ledger.metadata) : providerMeta;
    attempt.accounting = ledger
      ? {
          status: ledger.status,
          reservedMicrousd: ledger.reserved_microusd,
          chargedMicrousd: ledger.charged_microusd,
        }
      : null;
  ...
  console.log(
    `Completed exactly two live attempts. Gate: ${report.passed ? "PASS" : "FAIL — inspect safe report before deployment"}.`,
  );
} catch {
  report.interrupted = true;
  persist();
  console.error(
    "Test interrupted. Inspect the safe report before any further paid attempts. No automatic retries.",
  );
  process.exitCode = 1;
} finally {
  sqlite.close();
